...
function output = Phi_A(q)
    % create new robot
    robot2 = SerialLink(robotHandle.links(1:end-1));
    J = robotHandle.jacob0(q,'trans'); % Robot Jacobian in the global reference frame.
    J2 = robot2.jacob0(q(1:(end-1)),'trans');
    output = [J; J2 zeros(3,1)];
end
%------------- END OF CODE --------------
end
